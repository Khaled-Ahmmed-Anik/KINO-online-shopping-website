...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="card h-100 p-2 cart-color">
    <img width="150px" height="150px" src=${image} class="card-img-top p-3" alt="...">
    <div class="card-body rounded cart-body">
      <h5 class="card-title text-center fw-bolder">${product.title}</h5>
      <p class="card-text text-center">Category: ${product.category}</p>
      <h6 class="card-title text-center fw-bolder">Price:  $ ${product.price}</h6>
      <div class="row">
            <div class="col-lg-5">
              <p class="card-text text-center">Ratings: ${product.rating.rate}</p>
            </div>
            <div class="col-lg-7">
              <p class="card-text text-center">total review: ${product.rating.count}</p>
            </div>
          </div>
    </div>
    <div class="card-footer">
        <div class="row g-2">
          <div class="col-lg-8">
            <button onclick="addToCart(${product.id},${product.price})" type="button" class="btn btn-success text-left">Add to cart</button>
          </div>
          <div class="col-lg-4">
            <button type="button" onclick="showDetails(${product.id})" class="btn btn-danger text-right">Details</button>
          </div>
        </div>
    </div>
  </div>
     ...
     document.getElementById("all-product").appendChild(div);
  }
};
...
const showDetails= id =>{
  fetch(`https://fakestoreapi.com/products/${id}`)
            .then(res=>res.json())
            .then(data=> showModals(data.description) )
}

// pop up modals
const showModals= details=>{
...
const updatePrice = (id, value) => {
  ...
  document.getElementById(id).innerText = getRounded(total);
};

// set innerText function
const setInnerText = (id, value) => {
  document.getElementById(id).innerText = getRounded(value);
};
...
const updateTotal = () => {
  const grandTotal =getRounded(parseFloat(getInputValue("price")) + parseFloat(getInputValue("delivery-charge")) +
  parseFloat(getInputValue("total-tax")))
  document.getElementById("total").innerText = grandTotal;
};


// round value upto 2 decimal point
const getRounded= (value)=>{
  let converted=((value+0.005)*100)
...
const searchedProducts =()=>{
  ...
  fetch(`https://fakestoreapi.com/products/category/${category}`)
            ...
            document.getElementById("all-product").innerHTML='';
}
